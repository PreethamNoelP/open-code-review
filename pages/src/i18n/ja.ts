...
export const ja: TranslationKeys = {
  ...
  'scan.preview': 'プレビュー',
  'scan.batching': 'バッチ処理',
  'scan.toggles': 'ステージの切り替え',
  'scan.budget': 'トークン予算',
  'scan.flags': 'フラグリファレンス',
  'scan.overviewTitle': '概要',
  'scan.overviewDesc': '<code>ocr scan</code> は diff の代わりにファイル全体をレビューします。Git 履歴や意味のある diff は不要です。未知のコードベースの調査、移行前のディレクトリ調査、diff の対象がないプロジェクトのレビューに使用します。',
  'scan.overviewVsTitle': 'ocr scan と ocr review の違い',
  'scan.overviewVsReview': '<code>ocr review</code> は diff ベースです。ワークスペースのステージ済み/未ステージ/未追跡の変更、ブランチ範囲、または単一コミットなど、Git の変更をレビューします。diff を計算するために Git 履歴が必要です。',
  'scan.overviewVsScan': '<code>ocr scan</code> はファイル全体ベースです。Git 履歴の有無にかかわらず、ファイルの完全な内容をレビューします。非 git ディレクトリでも動作し、その場合は .gitignore に従ったファイルシステム走査にフォールバックします。',
  'scan.usageTitle': '基本的な使い方',
  'scan.usageWhole': 'リポジトリ全体をスキャン',
  'scan.usageWholeDesc': '--path を指定しない場合のデフォルトの動作。',
  'scan.usagePath': '特定のパスをスキャン',
  'scan.usagePathDesc': 'リポジトリ相対のディレクトリを指定してスキャン範囲を絞り込みます。',
  'scan.usageFile': '特定のファイルをスキャン',
  'scan.usageFileDesc': 'カンマ区切りの複数ファイルパスも指定できます。',
  'scan.usagePreviewLabel': 'LLM を呼び出さずにプレビュー',
  'scan.usagePreviewDesc': 'まずスキャン対象のファイル一覧を表示します —— LLM 呼び出しなし、コストなし。',
  'scan.previewTitle': '--preview による Dry-Run',
  'scan.previewDesc': '<code>--preview</code>（<code>-p</code>）は、スキャン対象となるすべてのファイルを列挙し、実際の実行と同じ包含/除外フィルタリングを適用しますが、LLM 呼び出しの前に停止します。ファイルの総数、レビュー対象と除外対象の数（および理由）、総行数を報告します。大きなリポジトリでトークンを消費する前にスコープを確認するのに役立ちます。',
  'scan.batchingTitle': 'バッチ処理戦略',
  'scan.batchingDesc': '<code>--batch</code> フラグは、ファイルがどのようにレビューバッチへグループ化されるかを制御します。スキャンテンプレートのデフォルトは <code>by-language</code> です。空の値や認識できない値は <code>none</code> にフォールバックします。',
  'scan.batchingNone': 'none',
  'scan.batchingNoneDesc': '各ファイルが独立したバッチになります —— グループ化なし。',
  'scan.batchingLang': 'by-language',
  'scan.batchingLangDesc': 'ファイル拡張子（小文字化）でグループ化されます。テンプレートのデフォルト。',
  'scan.batchingDir': 'by-directory',
  'scan.batchingDirDesc': 'リポジトリルート直下の第一階層ディレクトリでグループ化されます。ルート直下のファイルはそれぞれ独立したバッチになります。',
  'scan.togglesTitle': 'ステージの切り替え：--no-plan、--no-dedup、--no-summary',
  'scan.togglesDesc': 'スキャン実行には、コアとなるファイル単位のレビューに加えて3つのオプションステージがあります。各ステージは個別に無効化でき、レビュー品質とスピード・トークンコストのバランスを調整できます。',
  'scan.togglesPlanDesc': 'メインレビューの前に注力すべき範囲を決める、ファイル単位の PLAN_TASK 前処理をスキップします。ファイルごとに LLM 呼び出しを1回節約できますが、レビューの焦点が薄れる可能性があります。',
  'scan.togglesDedupDesc': 'バッチ単位の DEDUP_TASK をスキップします。通常は重複除去でまとめられる類似コメントも含め、すべての生コメントを保持します。',
  'scan.togglesSummaryDesc': '実行後の PROJECT_SUMMARY_TASK（すべてのバッチ完了後に生成されるリポジトリ単位の markdown サマリー）をスキップします。',
  'scan.budgetTitle': 'トークン予算：--max-tokens-budget',
  'scan.budgetDesc': 'ディスパッチ前に、<code>ocr scan</code> はおおまかなトークンコストの見積もりを表示します。<code>--max-tokens-budget</code> は LLM API から報告される入力+出力トークンの総使用量を制限します。累積使用量が上限を超えた時点で、次のバッチの開始前にディスパッチが停止します。デフォルトの <code>0</code> は無制限を意味します。',
  'scan.flagsTitle': 'フラグリファレンス',
  'scan.flagCol1': 'フラグ',
  'scan.flagCol2': '説明',
  'scan.flagCol3': 'デフォルト',
  'scan.flag1Desc': 'カンマ区切りのリポジトリ相対ディレクトリ/ファイル',
  'scan.flag1Default': 'リポジトリ全体',
  'scan.flag2Desc': 'カンマ区切りの gitignore 形式の除外パターン。rule.json の excludes と統合されます',
  'scan.flag3Desc': 'LLM を実行せずにスキャン対象ファイルをプレビュー',
  'scan.flag4Desc': 'トークン使用量（入力+出力）の合計を制限。超過するとディスパッチを停止（0 = 無制限）',
  'scan.flag5Desc': 'ファイル単位の PLAN_TASK 前処理をスキップ',
  'scan.flag6Desc': 'バッチ単位の DEDUP_TASK をスキップ',
  'scan.flag7Desc': '実行後の PROJECT_SUMMARY_TASK をスキップ',
  'scan.flag8Desc': 'バッチ処理戦略：none | by-language | by-directory',
  'scan.flag9Desc': '出力形式：text または json',
  'scan.flag10Desc': '最大並行ファイルスキャン数',
  'scan.flag11Desc': '並行タスクのタイムアウト（分）',
  'scan.flag12Desc': '出力対象：human（進捗表示）または agent（サマリーのみ）',
  'scan.flag13Desc': 'スキャンに関する任意の要件・業務背景情報',
  'scan.flag14Desc': 'ファイルあたりの最大ツール呼び出しラウンド数。テンプレートのデフォルトより大きい場合のみ有効',
  'scan.flag14Default': '内蔵',
  'scan.flag15Desc': '最大並行 git サブプロセス数',
  'scan.flag16Desc': 'カスタム JSON レビュールールのパス',
  'scan.flag17Desc': 'カスタム JSON ツール設定のパス',
  'scan.flag17Default': '内蔵',
  'scan.flag18Desc': 'スキャンするリポジトリまたはディレクトリのルート',
  'scan.flag18Default': '現在のディレクトリ',
  'scan.flagsNote': '注：<code>--max-tools</code> はスキャンテンプレートのファイル単位ツール呼び出し予算を引き上げる場合にのみ効果があります —— テンプレートのデフォルトより低い値を設定しても効果はありません。',
};
