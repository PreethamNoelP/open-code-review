...
export const zh: TranslationKeys = {
  ...
  'scan.batching': '批处理',
  'scan.toggles': '阶段开关',
  'scan.budget': 'Token 预算',
  'scan.flags': '参数参考',
  'scan.overviewTitle': '概览',
  'scan.overviewDesc': '<code>ocr scan</code> 审查整个文件而非 diff —— 无需 Git 历史或有意义的差异。适用于审计不熟悉的代码库、迁移前的目录扫描，或任何没有可比对内容的项目。',
  'scan.overviewVsTitle': 'ocr scan 与 ocr review 的区别',
  'scan.overviewVsReview': '<code>ocr review</code> 基于 diff：审查 Git 变更 —— 工作区中已暂存/未暂存/未跟踪的改动、分支范围，或单次提交。需要 Git 历史来计算差异。',
  'scan.overviewVsScan': '<code>ocr scan</code> 基于完整文件：无论是否存在 Git 历史，都审查完整的文件内容。它也可以在非 git 目录中工作，回退到遵循 .gitignore 的文件系统遍历。',
  'scan.usageTitle': '基本用法',
  'scan.usageWhole': '扫描整个仓库',
  'scan.usageWholeDesc': '未指定 --path 时的默认行为。',
  'scan.usagePath': '扫描指定路径',
  'scan.usagePathDesc': '传入相对于仓库根目录的目录以缩小扫描范围。',
  'scan.usageFile': '扫描指定文件',
  'scan.usageFileDesc': '也支持以逗号分隔的多个文件路径。',
  'scan.usagePreviewLabel': '预览而不调用 LLM',
  'scan.usagePreviewDesc': '先列出将被扫描的文件 —— 不调用 LLM，不产生费用。',
  'scan.previewTitle': '使用 --preview 进行 Dry-Run',
  'scan.previewDesc': '<code>--preview</code>（<code>-p</code>）会枚举扫描将涉及的每个文件，并应用与实际运行相同的包含/排除过滤逻辑，但会在调用 LLM 之前停止。它会报告文件总数、可审查与被排除的数量（及原因）以及总行数 —— 便于在大型仓库上花费 token 之前检查扫描范围。',
  'scan.batchingTitle': '批处理策略',
  'scan.batchingDesc': '<code>--batch</code> 参数控制文件如何被分组为审查批次。扫描模板默认使用 <code>by-language</code>；空值或无法识别的值会回退为 <code>none</code>。',
  'scan.batchingNone': 'none',
  'scan.batchingNoneDesc': '每个文件都是独立的批次 —— 不进行分组。',
  'scan.batchingLang': 'by-language',
  'scan.batchingLangDesc': '按文件扩展名（小写）分组。模板默认值。',
  'scan.batchingDir': 'by-directory',
  'scan.batchingDirDesc': '按仓库根目录下的第一级目录分组；根目录下的文件各自组成独立批次。',
  'scan.togglesTitle': '阶段开关：--no-plan、--no-dedup、--no-summary',
  'scan.togglesDesc': '除核心的按文件审查外，一次扫描运行还包含三个可选阶段。每个阶段都可以单独关闭，以在审查质量与速度、token 成本之间做取舍。',
  'scan.togglesPlanDesc': '跳过按文件的 PLAN_TASK 预处理阶段（该阶段在主审查前确定关注重点）。每个文件可节省一次 LLM 调用；可能降低审查聚焦度。',
  'scan.togglesDedupDesc': '跳过按批次的 DEDUP_TASK。保留所有原始评论，包括去重通常会合并的相似评论。',
  'scan.togglesSummaryDesc': '跳过运行结束后的 PROJECT_SUMMARY_TASK —— 即所有批次完成后生成的仓库级 markdown 摘要。',
  'scan.budgetTitle': 'Token 预算：--max-tokens-budget',
  'scan.budgetDesc': '在分发任务前，<code>ocr scan</code> 会打印一个粗略的 token 费用估算。<code>--max-tokens-budget</code> 限制 LLM API 报告的输入+输出 token 总用量；一旦累计用量超过该上限，将在开始下一批次前停止分发。默认值 <code>0</code> 表示无限制。',
  'scan.flagsTitle': '参数参考',
  'scan.flagCol1': '参数',
  'scan.flagCol2': '描述',
  'scan.flagCol3': '默认值',
  'scan.flag1Desc': '以逗号分隔的、相对于仓库的待扫描目录/文件',
  'scan.flag1Default': '整个仓库',
  'scan.flag2Desc': '以逗号分隔的 gitignore 风格排除模式；与 rule.json 中的 excludes 合并',
  'scan.flag3Desc': '预览将被扫描的文件，不运行 LLM',
  'scan.flag4Desc': '限制总 token 用量（输入+输出）；超出后停止分发（0 = 无限制）',
  'scan.flag5Desc': '跳过按文件的 PLAN_TASK 预处理',
  'scan.flag6Desc': '跳过按批次的 DEDUP_TASK',
  'scan.flag7Desc': '跳过运行结束后的 PROJECT_SUMMARY_TASK',
  'scan.flag8Desc': '批处理策略：none | by-language | by-directory',
  'scan.flag9Desc': '输出格式：text 或 json',
  'scan.flag10Desc': '最大并发文件扫描数',
  'scan.flag11Desc': '并发任务超时时间（分钟）',
  'scan.flag12Desc': '输出受众：human（显示进度）或 agent（仅摘要）',
  'scan.flag13Desc': '可选的需求/业务背景信息',
  'scan.flag14Desc': '每个文件的最大工具调用轮数；仅当大于模板默认值时生效',
  'scan.flag14Default': '内置',
  'scan.flag15Desc': '最大并发 git 子进程数',
  'scan.flag16Desc': '自定义 JSON 审查规则的路径',
  'scan.flag17Desc': '自定义 JSON 工具配置的路径',
  'scan.flag17Default': '内置',
  'scan.flag18Desc': '要扫描的仓库或目录根路径',
  'scan.flag18Default': '当前目录',
  'scan.flagsNote': '注意：<code>--max-tools</code> 仅会提高扫描模板中按文件的工具调用预算 —— 当设置值低于模板默认值时不生效。',
};
